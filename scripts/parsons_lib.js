...
// modified $ in the following to $question.
Each H5P.ParsonsJS is a question who has an independent container
/***********************************************/
...
var lines = $wrapper.$question.find("#" + item.id).html().split(/<br\s*\/?>/);
// go through all the lines
for (var i = 0; i < lines.length; i++) {
// add indents and get the text for the line (to remove the syntax highlight html elements)
executableCode += python_indents[item.indent] + $wrapper.$question.find("<span>" + lines[i] + "</span>").text() + "\n";
}
});
...
item = student_code[i],
line = $wrapper.$question.find("#" + item.id).text(), // code of the line
topBlock, bO;
...
if (item.indent < 0) {
  ...
  $wrapper.$question.find("#" + item.id).addClass("incorrectIndent");
...
for (var blockClose in close) {
  if (new RegExp(blockClose).test(line)) {
    ...
    if (!topBlock) {
      ...
      $wrapper.$question.find("#" + item.id).addClass("incorrectPosition");
...
for (i = 0; i < blocks.length; i++) {
  ...
  $wrapper.$question.find("#" + blocks[i].item.id).addClass("incorrectPosition");
}
}
...
if (parson.model_solution.length < student_code.length) {
  $wrapper.$question.find("#ul-" + elemId).addClass("incorrect");
